...
# @app.route("/friends_count")
# @login_required
# def friends_count(post_id):
#     db.friends_count(current_user.id,friend_id)
#    return redirect(url_for('feed')) 
...
@app.route('/add_friend_request/<int:receiver_id>' )
@login_required
def add_friend_request(receiver_id):
    db.add_friend_request(current_user.id,receiver_id)
    user = db.get_user_by_Id(receiver_id)[0]['BirdId']
    return redirect(url_for('profile', user=user))
   

@app.route('/cancel_friend_request/<int:receiver_id>' )
@login_required
def cancel_friend_request(receiver_id):
    db.cancel_friend_request(receiver_id,current_user.id)
    user = db.get_user_by_Id(receiver_id)[0]['BirdId']
    return redirect(url_for('profile', user=user))

@app.route('/cancel_a_friend_request/<int:sender_id>' )
@login_required
def cancel_a_friend_request(sender_id):
    db.cancel_a_friend_request(sender_id,current_user.id)
...
def search_users():
    ...
    search = db.search_user(search_input)
    user = db.get_user_by_Id(current_user.id)[0]
